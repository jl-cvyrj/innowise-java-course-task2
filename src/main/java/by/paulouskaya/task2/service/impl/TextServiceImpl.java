...
public class TextServiceImpl implements TextService {
    ...
    public int countLexemesInSentence(TextComposite sentence) {
        int count = 0;
    ...
    private void countWordsInSentence(TextComposite composite, Map<String, Integer> wordCount) {
        if (composite.getType() == TextComponentType.WORD) {
            String word = extractWord(composite);
            if (!word.isEmpty()) {
    ...
    private String extractWord(TextComposite wordComposite) {
        StringBuilder pureWord = new StringBuilder();
        extractWordRecursive(wordComposite, pureWord);
        return pureWord.toString().toLowerCase();
    }

    private void extractWordRecursive(TextComposite composite, StringBuilder pureWord) {
        if (composite.getType() == TextComponentType.LETTER) {
            pureWord.append(composite.toString());
            return;
        }

        for (TextComponent child : composite.getChildren()) {
            if (child instanceof TextComposite) {
                extractWordRecursive((TextComposite) child, pureWord);
            }
        }
    }
